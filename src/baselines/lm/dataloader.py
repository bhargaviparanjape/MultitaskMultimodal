...
class DataLoader(object):
    ...
    def load_dataset(self, question_path, answer_path,analysze= False):
        questions = json.load(open(question_path))['questions']
        sorted_questions = sorted(questions, key=lambda x :x['question_id'])
        metainfo = []
        datapoints = []  # currently concatenating only answers
        ...
        label2ans_path = pickle.load(open(self.args.label2answer_path, 'rb'))

        for question, answer in zip(sorted_questions, sorted_answers):
            image_id = question['image_id']
            assert image_id == answer['image_id']

            answers_labels = []
            answer_token_ids = []
            for label in answer['labels']:
                answer_tokens = label2ans_path[label]
                answers_labels.append(answer_tokens)
                answer_token_ids += self.preprocess_tokens(answer_tokens, init=True)


            data = {'image_id':image_id,
                    'question_id':question['question_id'],
                    'question':question['question'],
                    'answer':answers_labels,
                    #'question_ids':self.preprocess_tokens(question['question']),
                    'answer_ids':answer_token_ids}
            metainfo.append(data)
            if self.args.mode in ["question", "Ref-VQA"]:
                datapoints.append(self.preprocess_tokens(question['question'], init=True,analysze=analysze))
            elif self.args.mode == "answer":
                datapoints.append(answer_token_ids + [self.vocabulary.word2id['<eos>']])
            count +=1
    ...
    def load_captions(self, path, analysze= False, init=False):
        annotations = json.load(open(path))['refexps']
        datapoints = []  # currently concatenating only answers
        count = 0
        for annot in annotations:
            ...
            token_ids = []
            for token in tokens:
                token_ids.append(self.vocabulary.add_and_get_index(token, init=init))
            token_ids.append(self.vocabulary.word2id['<eos>'])
